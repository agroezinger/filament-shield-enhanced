...
### Fixed
- **`canShield()` threw instead of returning `false` for an ungenerated/mismatched permission**: `HasPageShield`, `HasResourceShield` and `HasComponentShield` all preferred Spatie's `hasPermissionTo()`/`hasAnyPermission()` over the Gate-routed `can()` when available on the user model. Those Spatie methods throw `PermissionDoesNotExist` for a permission name with zero matching rows — e.g. a declared action that was renamed, or whose generator hasn't run yet — whereas vanilla filament-shield's own `canAccess()` has always used `can()`, which resolves unknown abilities to `false`. Found while migrating rppt off its `filament-shield` fork onto this addon: a real page's Blade template called `canShield('editAllSettings')` while the page declared `'canEditAllSettings'` — a pre-existing, harmless typo under the fork (silently `false`) that became a hard crash under the addon. All three traits now use `can()` exclusively; a regression test covers a fully Spatie-equipped user checking a never-generated permission.
